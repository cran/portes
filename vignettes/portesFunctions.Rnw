...
\author{Esam Mahdi\\Qatar University \And
...
\code{("ARIMA forecast ARIMA Arima")}\footnote{The functions \code{Arima()} and \code{auto.arima()}
in the \code{R} package \code{forecast} produce an output with class \code{("ARIMA forecast ARIMA Arima")}.},
...
\code{"arima0"}, \code{"Arima"}, \code{("ARIMA forecast ARIMA Arima")}, \code{"lm"}, \code{("glm" "lm")},
...
\citep{HyndmanRazbash2019} is used to fit the best \ARIMA model based on the \AIC criterion to

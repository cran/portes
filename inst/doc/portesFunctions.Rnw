...
Some illustrative applications, including two examples from \citet{MahdiMcLeod2012}
using the \proglang{R} package \pkg{parallel} on a personal
PC running Windows 8.1 with a 64-bit operating system and four quad core CPUs,
are given \citep{VeraJansenSuppi2008}.
}

\Keywords{\ARMA models, \VARMA models, \SARIMA models, \GARCH models, \FGN models, \ARFIMA models,
...
the function \code{FitFGN()} from the \code{FGN} \code{R} package \citep{McLeodVeenstra2014},
